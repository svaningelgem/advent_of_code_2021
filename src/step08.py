...
def _diff(x: str, y: str) -> tuple[str, str]:
    return ("".join(set(x) - set(y)), "".join(set(y) - set(x)))
...
def _get_number(first: str, second: str) -> int:
    ...
    while not (
        ...
        unique_values = {k: next(iter(v)) for k, v in know.items() if len(v) == 1}
        ...
        for k, v in conversion_table.items():
            ...
            new_values = ["".join(set(old) - set(unique_values.values())) for old in v]
    ...
    translation_mapping = {next(iter(v)): k for k, v in know.items() if len(v) == 1}
    converted = ""
    for c in second:
        try:
